/**
 * MediaHub Profiles — управление профилями с синхронизацией через сервер.
 *
 ...
 * - Создание/переименование/удаление профилей синхронизируется с сервером
 * - Работает совместно с lm.js; является полной заменой стандартного profiles.js
 *
 * Установка: добавить ПОСЛЕ lm.js
 *
 * ⚠️  В init.conf Lampac необходимо отключить конфликтующие плагины:
 *       "initPlugins": { "sync": false, "bookmark": false }
 */
(function () {
    'use strict';
    ...
    // ── __NMSync (синхронизация настроек плагинов через /api/plugin-settings) ───

    if (!window.__NMSync) {
        (function () {
            function _token()   { return (window.Lampa && Lampa.Storage.get('mediahub_api_key', '')) || ''; }
            function _baseUrl() { return (window.Lampa && Lampa.Storage.get('base_url_mediahub', '')) || ''; }
            ...
            function _aesDecrypt(packed, token) {
                try {
                    ...
                    return _deriveKey(token).then(function (key) {
                        return crypto.subtle.decrypt({ name: 'AES-GCM', iv: fromB64(parts[0]) }, key, fromB64(parts[1]))
            ...
            function _xorEncrypt(value, token) {
                var str = JSON.stringify(value), out = '';
            ...
            function _xorDecrypt(packed, token) {
                try {
                    var str = atob(packed.slice(4)), out = '';
                    for (var i = 0; i < str.length; i++) out += String.fromCharCode(str.charCodeAt(i) ^ token.charCodeAt(i % token.length));
                    return Promise.resolve(JSON.parse(out));
                } catch (e) { return Promise.resolve(null); }
            }

            function _encrypt(value, token) { return _hasSubtle ? _aesEncrypt(value, token) : _xorEncrypt(value, token); }
            ...
            var _ws = null, _handlers = {}, _sensitive = {};
            ...
            function _applyMsg(plugin, key, value, msgProfileId) {
                ...
                if (_isSensitive(plugin, key) && _isEncrypted(value)) {
                    _decrypt(value, _token()).then(function (dec) { if (dec !== null) fn(key, dec); });
                } else {
                    fn(key, value);
                }
            }
            ...
            function _connect() {
                var token = _token(), baseUrl = _baseUrl();
                if (!token || !baseUrl || _ws) return;
                ...
                _ws.onmessage = function (e) {
                    try {
                        ...
                        if (msg.plugin && msg.key !== undefined) _applyMsg(msg.plugin, msg.key, msg.value, msg.lampa_profile_id || '');
                    } catch (ex) {}
                };
                _ws.onclose = function () { _ws = null; setTimeout(function () { if (_token()) _connect(); }, 5000); };
            }

            function _pull(plugin, callback) {
                var token = _token(), baseUrl = _baseUrl(), profileId = _profileId();
                if (!token || !baseUrl) { if (callback) callback([]); return; }
            ...
            window.__NMSync = {
                register: function (plugin, sensitiveKeys, applyFn, onPullComplete) {
                    ...
                    _connect();
                    _pull(plugin, onPullComplete ? function (sk) { onPullComplete(sk); } : null);
                },
                pullAll: function (callback) {
                    ...
                    plugins.forEach(function (p) { _pull(p, done); });
                },
                patch: function (plugin, key, value, callback) {
                    ...
                    var send = function (val) {
                        fetch(url, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key: key, value: val }) })
                            .then(function () { if (callback) callback(); })
                            .catch(function () { if (callback) callback(); });
                    };
                    if (_isSensitive(plugin, key)) _encrypt(value, token).then(send);
                    else send(value);
                }
            };
        })();
    ...
    // Очищаем стейл данные от других плагинов профилей.
    (function sanitize() {
        try {
            var rawActive = window.localStorage.getItem('lm_active_profile');
            if (rawActive) {
                var parsed = JSON.parse(rawActive);
                if (parsed && parsed.profile_id && /^\d+$/.test(String(parsed.profile_id))) {
                    window.localStorage.removeItem('lm_active_profile');
                }
            }
        } catch (e) {}
    })();
    ...
    var BASE_URL = (function () {
        ...
        return params.get('base_url')
            || Lampa.Storage.get('base_url_mediahub', '')
            || src.replace(/\/lm_profiles\.js.*$/, '');
    })();

    var FV_PREFIX  = 'lm_fv_';
    var FAV_PREFIX = 'lm_fav_';
    var ACTIVE_KEY = 'lm_active_profile';
    ...
    function getToken() { return Lampa.Storage.get('mediahub_api_key', ''); }
    ...
    function getActiveProfile() { return Lampa.Storage.get(ACTIVE_KEY, null); }
    ...
    function setActiveProfile(profile) {
        ...
        Lampa.Storage.set('lm_profile_name',   apiName);
    }
    ...
    function refreshPage() {
        try {
            ...
            if (activity) { activity.page = 1; Lampa.Activity.replace(activity); }
        } catch (e) {}
    }
    ...
    function scheduleSaveFavorite() {
        ...
        _saveFavTimer = setTimeout(function () {
            ...
            fetch(url, {
                method:  'PUT',
                headers: { 'Content-Type': 'application/json' },
                body:    JSON.stringify({ favorite: Lampa.Storage.get('favorite', {}) })
            }).catch(function () {});
        }, 2000);
    }

    function applyFileView(profile, fileView) {
        Lampa.Storage.set(timelineKey(), fileView || {});
        if (Lampa.Timeline && Lampa.Timeline.read) Lampa.Timeline.read();
    ...
    function switchToProfile(profile) {
        ...
        if (current) {
            Lampa.Storage.set(FV_PREFIX  + current.profile_id, Lampa.Storage.get(timelineKey(), {}));
        ...
        if (getToken()) {
            fetchFileView(profile.profile_id, function (fv) {
    ...
    var AVAILABLE_ICONS = [
        { id: 'id1',  label: 'Мужчина'     }, { id: 'id2',  label: 'Женщина'     },
        { id: 'id3',  label: 'Мальчик'     }, { id: 'id4',  label: 'Девочка'     },
        { id: 'id5',  label: 'Мышь'        }, { id: 'id6',  label: 'Безликий'    },
        { id: 'id7',  label: 'Малыш'       }, { id: 'id8',  label: 'Кот'         },
        { id: 'id9',  label: 'Робот'       }, { id: 'id10', label: 'Принцесса'   },
        { id: 'id11', label: 'Дедушка'     }, { id: 'id12', label: 'Ниндзя'      },
        { id: 'id13', label: 'Хоккеист'    }, { id: 'id14', label: 'Тхэквондист' },
        { id: 'id15', label: 'Балерина'    }, { id: 'id16', label: 'Супергерой'  },
        { id: 'id17', label: 'Пират'       }, { id: 'id18', label: 'Астронавт'   }
    ];

    var PNG_ICONS = { id1: 1, id2: 1, id3: 1, id4: 1, id5: 1, id6: 1, id7: 1, id13: 1 };

    function iconUrl(iconId) {
        if (!iconId) return null;
        var ext = PNG_ICONS[iconId] ? '.png' : '.svg';
        return BASE_URL + '/img/profiles/' + iconId + ext;
    }

    function iconHtml(iconId) {
        var url = iconUrl(iconId);
        if (!url) return DEFAULT_ICON_SVG;
        return '<img src="' + url + '" style="width:2em;height:2em;border-radius:50%;object-fit:cover" onerror="this.style.display=\'none\'">';
    }
    ...
    function updateButton(profile) {
        ...
        if (!document.contains(_btn[0])) {
            var $orig = $('.open--profile').not('.lm-profile-btn');
            if ($orig.length) $orig.replaceWith(_btn);
            else { var $h = $('.head__actions').first(); if ($h.length) $h.prepend(_btn); }
        }
        _btn.attr('title', profile ? profile.name : 'Профили');
        _btn.empty();
        if (profile && profile.icon) {
            _btn.append('<img src="' + iconUrl(profile.icon) + '" style="width:2.2em;height:2.2em;border-radius:50%;object-fit:cover;display:block" onerror="this.style.display=\'none\'">');
        } else {
    ...
    function renderButton(profile) {
        if (_btn) { updateButton(profile); return; }
        _btn = $('<div class="head__action selector open--profile lm-profile-btn" title=""></div>');
        updateButton(profile);
        _btn.on('hover:enter hover:click hover:touch click', function () { openSelector(); });
        _btn[0].addEventListener('touchend', function (e) { e.preventDefault(); e.stopPropagation(); openSelector(); }, { passive: false });
        var $orig = $('.open--profile');
        if ($orig.length) $orig.replaceWith(_btn);
        else { var $head = $('.head__actions').first(); if ($head.length) $head.prepend(_btn); }
    }

    // ── Profile selector UI ───────────────────────────────────────────────────────

    var _allProfiles = [], _loading = false;
        ...
        fetchProfiles(function (data) {
            ...
            Lampa.Select.show({
                ...
                onSelect: function (item) {
                    if (item.create) { Lampa.Select.close(); openCreateDialog(); }
    ...
    function openProfileMenu(profile) {
        ...
        var items    = [];
        if (!isActive && _allProfiles.length > 1) items.push({ title: 'Переключиться', action: 'switch' });
        ...
        Lampa.Select.show({
            ...
            items: items,
            onSelect: function (item) {
                if      (item.action === 'switch') { Lampa.Select.close(); switchToProfile(profile); }
                else if (item.action === 'icon')   openIconPicker(profile);
                else if (item.action === 'rename')  { Lampa.Select.close(); setTimeout(function () { openRenameDialog(profile); }, 100); }
                else if (item.action === 'delete')  confirmDelete(profile);
            },
            onBack: function () { openSelector(); }
        });
    }

    function openIconPicker(profile) {
        var items = AVAILABLE_ICONS.map(function (ic) {
            return { title: ic.label, template: 'selectbox_icon', icon: iconHtml(ic.id), iconId: ic.id, selected: profile.icon === ic.id };
        });
        items.unshift({ title: 'Без иконки', template: 'selectbox_icon', icon: DEFAULT_ICON_SVG, iconId: null, selected: !profile.icon });
            ...
            onSelect: function (item) {
                ...
                fetch(apiUrl('profiles/' + encodeURIComponent(profile.profile_id)), {
                    ...
                    .then(function () {
                        ...
                        var active = getActiveProfile();
                        if (active && active.profile_id === profile.profile_id) { active.icon = newIcon; setActiveProfile(active); }
                        renderButton(getActiveProfile());
                        Lampa.Noty.show('Иконка обновлена');
                        openProfileMenu(profile);
                    })
                    .catch(function () { Lampa.Noty.show('Ошибка сохранения иконки'); openProfileMenu(profile); });
            },
            onBack: function () { openProfileMenu(profile); }
        });
    }

    function focusInput() {
        setTimeout(function () {
            var inp = document.querySelector('.input__input, input[type="text"]');
            if (inp) inp.focus();
        }, 200);
    }

    function openCreateDialog() {
        Lampa.Input.edit({ title: 'Новый профиль', value: '', free: true, nosave: true }, function (name) {
            if (!name || !name.trim()) return;
            var trimmedName = name.trim();
            var iconItems = AVAILABLE_ICONS.map(function (ic) {
                return { title: ic.label, template: 'selectbox_icon', icon: iconHtml(ic.id), iconId: ic.id };
            });
            iconItems.unshift({ title: 'Без иконки', template: 'selectbox_icon', icon: DEFAULT_ICON_SVG, iconId: null });
            ...
            Lampa.Select.show({
                ...
                onSelect: function (item) {
                    ...
                    }, function (err) { Lampa.Noty.show('Ошибка: ' + (err || 'не удалось создать профиль')); });
                },
                onBack: function () {
                    apiCreateProfile(trimmedName, null, function (result) {
    ...
    function openRenameDialog(profile) {
        Lampa.Select.close();
        Lampa.Input.edit({ title: 'Переименовать', value: profile.name, free: true, nosave: true }, function (name) {
            if (!name || !name.trim()) return;
            var trimmed = name.trim();
            var isEmpty = !profile.profile_id;
            var url     = isEmpty ? apiUrl('profiles') : apiUrl('profiles/' + encodeURIComponent(profile.profile_id));
            fetch(url, { method: isEmpty ? 'POST' : 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: trimmed }) })
                .then(function (r) { return r.ok ? r.json() : Promise.reject(); })
                .then(function (result) {
                    var newId  = result.profile_id || profile.profile_id;
                    var active = getActiveProfile();
                    if (active && active.profile_id === profile.profile_id) { active.name = trimmed; active.profile_id = newId; setActiveProfile(active); }
                ...
                .catch(function () { Lampa.Noty.show('Ошибка переименования'); try { Lampa.Controller.toggle('content'); } catch (e) {} });
        });
        focusInput();
    }
    ...
    function confirmDelete(profile) {
        Lampa.Select.show({
            ...
            onSelect: function (item) {
                if (item.action === 'no') { openProfileMenu(profile); return; }
                Lampa.Select.close();
                ...
                }, function (err) { Lampa.Noty.show('Ошибка: ' + (err || 'не удалось удалить профиль')); });
            },
            onBack: function () { openProfileMenu(profile); }
        });
    }

    // ── История (компонент + пункт меню) ─────────────────────────────────────────
    ...
    function registerHistoryComponent() {
        if (Lampa.Component.get('lm_history')) return;
        Lampa.Component.add('lm_history', function (object) {
            ...
            comp.use({
                onCreate: function () {
                    ...
                    var reload = function () {
                        ...
                        object.page = 1;
                        self.activity.loader(true);
                        fetchHistory(object, function (result) { self.build(result); self.activity.loader(false); },
                                              function ()       { self.empty(); self.activity.loader(false); });
                    };

                    fetchHistory(object, function (result) { self.build(result); self.activity.loader(false); },
                                          function ()       { self.empty(); self.activity.loader(false); });

                    Lampa.Listener.follow('profile', function (e) { if (e.type === 'changed') reload(); });
                },
                onNext: function (resolve, reject) { fetchHistory(object, resolve, reject); },
                    ...
                    item.use({
                        onCreate: function () {
                            ...
                            if (isTv && watched != null && total > 0) {
                                ...
                                if (remaining > 0) {
                                    cardImg.append($('<div>').css({ position:'absolute', top:'4px', right:'4px', background:'rgba(0,0,0,.75)', color:'#fff', borderRadius:'4px', padding:'2px 5px', fontSize:'11px', fontWeight:'600', lineHeight:'1.4' }).text(remaining));
                                }
                                cardImg.append($('<div>').css({ position:'absolute', bottom:'8px', left:'4px', background:'#2d8a4e', color:'#fff', borderRadius:'4px', padding:'2px 5px', fontSize:'11px', fontWeight:'600', lineHeight:'1.4' }).text(watched + '/' + total));
                            }
                        },
                        onEnter: function () {
    ...
    function addHistoryMenuItem() {
        if ($('.menu__item[data-action="lm_history"]').length) return;
        var menuItem = $('<li data-action="lm_history" class="menu__item selector">'
            + '<div class="menu__ico">' + HISTORY_ICON_SVG + '</div>'
            + '<div class="menu__text">История</div>'
            + '</li>');
        menuItem.on('hover:enter', function () {
            var profile = getActiveProfile();
            Lampa.Activity.push({ title: 'История', component: 'lm_history', page: 1, profile_id: profile ? profile.profile_id : '' });
        });
    ...
    function init() {
        ...
        // Патчим bookmark.js чтобы не перезаписывал наши закладки по WebSocket
        if (!Lampa.Favorite._lm_patched) {
            Lampa.Favorite._lm_patched = true;
        ...
        if (_cachedProfile) setTimeout(function () { renderButton(_cachedProfile); }, 300);

        // Перезагружаем активность если вернулись на неё с другим профилем
        Lampa.Listener.follow('activity', function (e) {
            ...
            if (activity._lmLastProfileId === undefined) { activity._lmLastProfileId = currentId; return; }
            if (activity._lmLastProfileId !== currentId) {
                activity._lmLastProfileId = currentId;
                activity.page = 1;
        ...
        fetchProfiles(function (data) {
            ...
            if (saved) {
                for (var i = 0; i < profiles.length; i++) {
                    if (profiles[i].profile_id === saved.profile_id) { active = profiles[i]; break; }
                }
            }
            ...
            if (active) {
                ...
                fetchFileView(active.profile_id, function (fv) {
                    ...
                    if (Lampa.Timeline && Lampa.Timeline.read) Lampa.Timeline.read();
                    fetchFavorite(active.profile_id, function (fav) { applyFavorite(fav); }, function () {});
                }, function () {});
            ...
            // Фоновое обновление онгоинг-сериалов (fire-and-forget, раз в сутки на сервере)
    ...
    // При открытии full-карточки восстанавливаем наши таймкоды
    function listenFullCardOpen() {
        ...
        // Обновляем эпизоды при открытии карточки сериала
