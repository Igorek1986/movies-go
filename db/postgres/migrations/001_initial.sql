...
-- Replaces the Python/SQLAlchemy create_all.
...
CREATE TABLE IF NOT EXISTS users (
    ...
    backup_codes          TEXT,           -- JSON list of SHA-256 hex digests
    -- Premium
...
CREATE TABLE IF NOT EXISTS lampa_profiles (
    ...
    icon             VARCHAR(20),
    favorite         TEXT,           -- JSON: Lampa favorite object
...
-- ─── Timecodes (watch progress) ───────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS timecodes (
    id               BIGSERIAL    PRIMARY KEY,
    device_id        BIGINT       NOT NULL REFERENCES devices(id) ON DELETE CASCADE,
    lampa_profile_id VARCHAR(100) NOT NULL DEFAULT '',
    card_id          VARCHAR(100) NOT NULL,   -- "{tmdb_id}_movie" | "{tmdb_id}_tv"
    item             VARCHAR(100) NOT NULL,   -- lampa_hash (episode/movie hash)
    data             TEXT         NOT NULL,   -- JSON: {duration, time, percent}
    counted_at       DATE,
...
-- ─── Media cards (TMDB + torrent metadata) ────────────────────────────────────
CREATE TABLE IF NOT EXISTS media_cards (
    card_id              VARCHAR(100) PRIMARY KEY,   -- "{tmdb_id}_{media_type}"
...
CREATE TABLE IF NOT EXISTS myshows_user_status (
    ...
    lampa_profile_id VARCHAR(100) NOT NULL DEFAULT '',
    item_id          BIGINT       NOT NULL REFERENCES myshows_items(id) ON DELETE CASCADE,
    cache_type       VARCHAR(20)  NOT NULL,   -- watchlist / watched / cancelled
    updated_at       TIMESTAMPTZ  NOT NULL DEFAULT now(),
    PRIMARY KEY (device_id, lampa_profile_id, item_id)
);
