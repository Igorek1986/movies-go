...
do_update() {
    ...
    # Пересборка оставляет старый образ как висячий (<none>) и копит build
    # cache (промежуточные слои) — подчищаем оба, чтобы не набегали гигабайты
    # за несколько обновлений. Только висячие образы, рабочие не трогаем.
